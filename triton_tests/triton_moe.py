...
def _fused_moe_triton_kernel(
    x_ptr, out_ptr,
    gate_w_ptr, gate_b_ptr, gate_out_w_ptr, gate_out_b_ptr,
    mlp_w1_ptr, mlp_b1_ptr, mlp_w2_ptr, mlp_b2_ptr, mlp_w3_ptr, mlp_b3_ptr,
    num_experts, num_selected_experts, num_default_experts,
...
class MoeTritonLayer(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        _fused_moe_triton_kernel[grid](
            x, out,
            self.gate_w, self.gate_b, self.gate_out_w, self.gate_out_b,
            self.mlp_w1, self.mlp_b1, self.mlp_w2, self.mlp_b2, self.mlp_w3, self.mlp_b3,
            self.num_experts, self.num_selected_experts, 0,
            BLOCK_SIZE_M=128,
            BLOCK_SIZE_E=4,
        )
