# Copied from here as a basic test nerf

# https://github.com/MaximeVandegar/Papers-in-100-Lines-of-Code/blob/main/InfoNeRF_Ray_Entropy_Minimization_for_Few_Shot_Neural_Volume_Rendering/infonerf.p#y
import torch
import numpy as np
...
class NerfModel(nn.Module):
    def __init__(self, embedding_dim_pos=20, embedding_dim_direction=8, hidden_dim=128):
        ...
        self.block3 = nn.Sequential(
            nn.Linear(embedding_dim_direction * 3 + hidden_dim, hidden_dim // 2),
            nn.ReLU(),
        )
...
def train(nerf_model, optimizer, scheduler, data_loader, device="cpu", hn=0, hf=1, nb_epochs=int(1e5), nb_bins=192, lambda_mul=4, T=0.1):
    training_loss = []
    for _ in range(nb_epochs):
        for batch in tqdm(data_loader):
            ...
            regenerated_px_values, regularization = render_rays(nerf_model, ray_origins, ray_directions, hn=hn, hf=hf, nb_bins=nb_bins, T=T)
